...
public class ViewJspWriter implements Constant {
	private static final Logger log = Logger.getLogger(ViewJspWriter.class);
	...
	public static void genCode(CodeFactory factory, Object instance, String webProjectName, String serviceProject) throws Exception {
		log.info("--谷登科技JAVA代码生成工具----生成view.jsp--------开始--------");
		try {
			...
			String outPutFileName = actionFileOutPath + "view.jsp";
			Writer out1 = new OutputStreamWriter(new FileOutputStream(outPutFileName), "UTF-8");
			t.process(root, out1);
		} catch (Exception e) {
			e.printStackTrace();
		}
		log.info("--谷登科技JAVA代码生成工具----生成view.jsp--------结束--------");
	}
