...
<div id="datagrid-tool-bar" style="padding:5px;height:auto">
	...
	<div style="margin-bottom:5px">
		...
		<a class="easyui-linkbutton icon-add-btn" iconCls="icon-edit" id="btn-edit" >新增</a>
...
function optformat(value,row,index){
	...
	html+="<a class='operate' href='javascript:;' onclick=deleteObj('"+row.${pkName}+"');>删除</a>";
	return html;
}
...
$("#exportData").click(function(){
	...
	var paramList = <#list fields as item><#if item_index == 0 && item_has_next>'${item}=' + ${item} + <#elseif item_index == 0 >'${item}=' + ${item};<#elseif item_index != 0 && item_has_next>'&${item}=' + ${item} +<#else>'&${item}=' + ${item};</#if></#list>
	$.ajax({
