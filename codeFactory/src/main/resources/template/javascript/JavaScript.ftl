...
function loadList(params){
	...
	$('#datagrid-table').datagrid({
		...
		columns:[[
			<#list fields as item>
			<#if item == pkName>{field:'${item}',title:'',width:50,checkbox:true}<#if item_has_next>,</#if></#if><#if item != pkName>{field:'${item}',title:'${item}',width:100,align:'center'}<#if item_has_next>,</#if></#if>
			</#list>
		]]
	}); 
	//分页加载
